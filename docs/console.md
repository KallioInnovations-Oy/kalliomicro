...
> ⚠ **No overlap protection (as of 2026-07-14).** If a task runs longer than its schedule interval, the next `schedule:run` tick starts a second concurrent instance. Before scheduling anything slow or non-idempotent, add a lock (e.g. MySQL `GET_LOCK` held by the DB connection — auto-releases if the process dies) around task execution. Derived deployments learned this in production; treat it as required for any task touching shared state.
