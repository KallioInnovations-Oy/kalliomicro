...
- **Mechanism vs. policy:** the base deliberately ships mechanisms without policies — impersonation without an authorization gate, a scheduler without overlap locking, login without throttling, sessions without a shared store, OAuth without account-provisioning rules. These are scope boundaries, not defects: each policy belongs to the deployment that knows its requirements. Decide each consciously before production (see the scope notes in [auth.md](auth.md) and [console.md](console.md)).
